...
export async function askHandler(
  ...
  function onChunk(chunk: string): void {
    if (config.debug) console.error(`[hermes-mcp] ${chunk.trimEnd()}`);
    if (emit) emit(chunk);
  }
  ...
  try {
    const result = await runCli(args, {
      ...
      debug: config.debug,
      debugPrefix: "[hermes-mcp]",
      onChunk,
    });
    return mcpText(result.stdout);
  } catch (e) {
    if (e instanceof CliTimeoutError) return mcpError(`hermes timed out after ${timeoutMs}ms`);
