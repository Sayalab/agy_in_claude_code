...
export async function askHandler(
  ...
  function onChunk(chunk: string): void {
    if (config.debug) console.error(`[codex-mcp] ${chunk.trimEnd()}`);
    if (emit) emit(chunk);
  }
  ...
  try {
    const result = await runCli(args, {
      ...
      debug: config.debug,
      debugPrefix: "[codex-mcp]",
      onChunk,
    });
    return mcpText(result.stdout);
  } catch (e) {
    if (e instanceof CliTimeoutError) return mcpError(`codex timed out after ${timeoutMs}ms`);
